 ...
 * -I ./ -Wall helper_test.c ../../tools/lib/bpf/bpf.c -o \
 *  helper_bpf
 *
 * TEST:
 * ./test_runner.sh
...
static void prog_load(void)
{
	...
	struct bpf_insn prog[] = {
		...
		/*
		 * pc9. if r0 != 0x0, go to pc+16, since we have the cookie
		 * stored already
		 ...
		 * be done by directly write to pointers insdead of using
	...
	prog_fd = bpf_load_program(BPF_PROG_TYPE_SOCKET_FILTER, prog, ARRAY_SIZE(prog),
...
static void prog_attach_iptables(char *file)
{
	...
	ret = system(rules);
	if (ret < 0) {
		printf("iptable rule update failed: %d/n", WEXITSTATUS(ret));
		exit(-1);
	}
}
...
static void print_table(void)
{
	...
	while (bpf_map_get_next_key(map_fd, &curN, &nextN) > -1) {
		...
		if (res < 0) {
			printf("fail to get entry value of Key: %u\n"
				"Error: %s\n", curN, strerror(errno));
		} else {
