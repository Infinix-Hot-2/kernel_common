...
static int invalidate_vbmeta(dev_t vbmeta_devt)
{
	...
	if (invalidate_vbmeta_submit(bio, bdev, rw, page)) {
		ret = -1;
	...
	bdev = blkdev_get_by_dev(vbmeta_devt, dev_mode,
				 invalidate_vbmeta);
	if (IS_ERR(bdev)) {
		DMERR("invalidate_vbmeta: could not open device for reading");
		dev_mode = 0;
		ret = -1;
...
void dm_verity_avb_error_handler(void)
{
	...
	if (!dev) {
		DMERR("No matching partition for device: %s", avb_vbmeta_device);
		goto fail_no_dev;
	}
...
{
	DMINFO("AVB error handler initialized with vbmeta device: %s", avb_vbmeta_device);
	return 0;
}
