...
/* Includes implementation of sw_sync adapted for goldfish_sync. */
#include "goldfish_sync_timeline_fence.c"
...
goldfish_sync_timeline_create(void)
{
	...
	struct sync_timeline *res_sync_tl = NULL;
							...
							uint32_t val)
{
	...
	syncpt = goldfish_sync_pt_create_internal(tl, sizeof(struct sync_pt) + 4, val);
	if (!syncpt) {
	...
	DPRINT("installing sync fence into fd %d sync_file_obj=0x%p", fd, sync_file_obj);
