 ...
 */
struct sync_pt
*goldfish_sync_pt_create(struct goldfish_sync_timeline *obj,
			 int size, unsigned int value);

/**
 * goldfish_sync_timeline_signal() - signals a status change on a sync_timeline
...
/**
 * goldfish_sync_timeline_put() - dec refcount of a sync_timeline
