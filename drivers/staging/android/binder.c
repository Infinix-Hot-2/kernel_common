			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_DECREFS: {
			...
			if (get_user_preempt_disabled(target,
						      (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
		...
		case BC_ACQUIRE_DONE: {
			...
			if (get_user_preempt_disabled(
			    node_ptr, (binder_uintptr_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(binder_uintptr_t);
			if (get_user_preempt_disabled(
			    cookie, (binder_uintptr_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(binder_uintptr_t);
		...
		case BC_FREE_BUFFER: {
			...
			if (get_user_preempt_disabled(
			    data_ptr, (binder_uintptr_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(binder_uintptr_t);
		...
		case BC_CLEAR_DEATH_NOTIFICATION: {
			...
			if (cmd == BC_REQUEST_DEATH_NOTIFICATION) {
				...
				death = kzalloc_preempt_disabled(
				    sizeof(*death));
				if (death == NULL) {
...
retry:
	...
	if (thread->return_error != BR_OK && ptr < end) {
		if (thread->return_error2 != BR_OK) {
			if (put_user_preempt_disabled(thread->return_error2,
						      (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
		...
		if (put_user_preempt_disabled(thread->return_error,
					      (uint32_t __user *)ptr))
			return -EFAULT;
		ptr += sizeof(uint32_t);
	...
	while (1) {
		...
		case BINDER_WORK_TRANSACTION_COMPLETE: {
			...
			if (put_user_preempt_disabled(cmd,
						      (uint32_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(uint32_t);
		...
		case BINDER_WORK_NODE: {
			...
			if (cmd != BR_NOOP) {
				if (put_user_preempt_disabled(
				    cmd, (uint32_t __user *)ptr))
					return -EFAULT;
				ptr += sizeof(uint32_t);
				if (put_user_preempt_disabled(
				    node->ptr, (binder_uintptr_t __user *)ptr))
					return -EFAULT;
				ptr += sizeof(binder_uintptr_t);
				if (put_user_preempt_disabled(
				    node->cookie,
		...
		case BINDER_WORK_CLEAR_DEATH_NOTIFICATION: {
			...
			ptr += sizeof(uint32_t);
			if (put_user_preempt_disabled(
			    death->cookie, (binder_uintptr_t __user *)ptr))
				return -EFAULT;
			ptr += sizeof(binder_uintptr_t);
			binder_stat_br(proc, thread, cmd);
			binder_debug(BINDER_DEBUG_DEATH_NOTIFICATION,
	...
	*consumed = ptr - buffer;
	if (proc->requested_threads + proc->ready_threads == 0 &&
		...
		binder_debug(BINDER_DEBUG_THREADS,
			     "%d:%d BR_SPAWN_LOOPER\n",
			     proc->pid, thread->pid);
		if (put_user_preempt_disabled(BR_SPAWN_LOOPER,
				...
				struct binder_thread *thread)
{
	...
	if (bwr.write_size > 0) {
		...
		if (ret < 0) {
			bwr.read_consumed = 0;
			if (copy_to_user_preempt_disabled(ubuf, &bwr,
							  sizeof(bwr)))
				ret = -EFAULT;
			goto out;
		}
