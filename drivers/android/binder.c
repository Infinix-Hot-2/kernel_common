					      ...
					      int is_async)
{
	...
	if (size < data_size || size < offsets_size || size < sg_buffers_size) {
	...
	buffer->sg_buffers_size = sg_buffers_size;
			    ...
			    struct binder_buffer *buffer)
{
	...
	size = ALIGN(buffer->data_size, sizeof(void *)) +
		...
		ALIGN(buffer->sg_buffers_size, sizeof(void *));
			       ...
			       binder_size_t sg_buffers_size)
{
	...
	t->buffer = binder_alloc_buf(target_proc, tr->data_size,
		tr->offsets_size, sg_buffers_size,
	...
	sg_buf_end = sg_bufp + sg_buffers_size;
	off_min = 0;
	for (; offp < off_end; offp++) {
		struct binder_object_header *hdr;
