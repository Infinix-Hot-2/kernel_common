...
struct binder_context {
	...
	struct dentry *debugfs_dir_entry_root;
	struct dentry *debugfs_dir_entry_proc;
					   ...
					   binder_uintptr_t cookie)
{
	...
	node->debug_id = ++proc->context->binder_last_id;
	node->proc = proc;
	node->context = proc->context;
	node->ptr = ptr;
	node->cookie = cookie;
	...
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
			    ...
			    !(node->proc &&
			      node == node->context->binder_context_mgr_node &&
			      node->has_strong_ref)) {
						  ...
						  struct binder_node *node)
{
	...
	new_ref->debug_id = ++proc->context->binder_last_id;
	new_ref->proc = proc;
	new_ref->node = node;
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	t->debug_id = ++context->binder_last_id;
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	if (proc->context->debugfs_dir_entry_proc) {
		char strbuf[11];

		snprintf(strbuf, sizeof(strbuf), "%u", proc->pid);
		proc->debugfs_entry = debugfs_create_file(strbuf, S_IRUGO,
			proc->context->debugfs_dir_entry_proc, proc,
			&binder_proc_fops);
	}
...
static int binder_proc_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, &binder_devices, hlist) {
		if (do_lock)
			binder_lock(&device->context, __func__);

		hlist_for_each_entry(itr, &device->context.binder_procs,
				     proc_node) {
			if (itr == proc) {
		...
		if (do_lock)
			binder_unlock(&device->context, __func__);
		if (found)
			break;
	}
	return 0;
...
static void __init free_binder_device(struct binder_device *device)
{
	...
	debugfs_remove_recursive(device->context.binder_debugfs_dir_entry_root);
	kfree(device);
}
...
static int __init init_binder_device(const char *name)
{
	...
	context->debugfs_dir_entry_root = debugfs_create_dir(name, NULL);
	context->debugfs_dir_entry_proc =
		debugfs_create_dir("proc", context->debugfs_dir_entry_root);

	debugfs_create_file("state",
			    S_IRUGO,
			    context->debugfs_dir_entry_root,
			    context,
			    &binder_state_fops);
	...
	debugfs_create_file("transaction_log",
			    S_IRUGO,
			    context->debugfs_dir_entry_root,
			    &context->binder_transaction_log,
