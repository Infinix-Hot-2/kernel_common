...
{
	struct binder_proc *proc, *existing_proc;
	...
	hlist_for_each_entry(existing_proc, &binder_procs, proc_node) {
		if (existing_proc->pid == current->group_leader->pid) {
			proc_exists = true;
			break;
		}
	}
	...
	if (!proc_exists && binder_debugfs_dir_entry_proc) {
		char strbuf[11];
