 ...
 * @index:	index in offset array at which the binder_buffer_object is located
 * @start:	points to the start of the offset array
 * @end:	points to the first invalid index of the offset array
 *
 * Return:	If @offset is within the valid range of the offset array
 *		described by @start and @end, and if there's a valid
							...
							binder_size_t *end)
{
	...
	binder_size_t *offset_addr = start + offset;
	if (offset_addr < start || offset_addr >= end)
		return NULL;

	buffer_obj = (struct binder_buffer_object *)(b->data + *offset_addr);
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	sg_bufp = (uint8_t *)(PTR_ALIGN(off_end, sizeof(void *)));
	...
	for (; offp < off_end; offp++) {
		...
		case BINDER_TYPE_PTR: {
			...
			parent = binder_validate_ptr(t->buffer, bp->parent,
						     off_start, offp);
			if (parent == NULL) {
				binder_user_error("%d:%d got transaction with invalid parent offset or type\n",
					  proc->pid, thread->pid);
