...
{
	atomic_inc(&binder_obj_stats.obj_zombie[type]);
}

static inline void binder_stats_delete_zombie(enum binder_stat_types type)
{
	atomic_dec(&binder_obj_stats.obj_zombie[type]);
...
struct binder_context {
	...
	struct mutex binder_dead_nodes_lock;
...
static int binder_ioctl_set_ctx_mgr(struct file *filp)
{
	...
	mutex_lock(&context->binder_context_mgr_node_lock);
...
out:
	mutex_unlock(&context->binder_context_mgr_node_lock);
	return ret;
}
...
static void add_binder_stats(struct binder_stats *from, struct binder_stats *to)
{
	...
	for (i = 0; i < ARRAY_SIZE(to->bc); i++)
		atomic_add(atomic_read(&from->bc[i]), &to->bc[i]);

	for (i = 0; i < ARRAY_SIZE(to->br); i++)
		atomic_add(atomic_read(&from->br[i]), &to->br[i]);
}
			       ...
			       struct binder_obj_stats *obj_stats)
{
	...
	for (i = 0; i < ARRAY_SIZE(obj_stats->obj_created); i++) {
		...
		int obj_zombie = atomic_read(&obj_stats->obj_zombie[i]);

		if (obj_created || obj_deleted || obj_zombie)
			seq_printf(m, "%s%s: active %d zombie %d total %d\n",
				   prefix,
				   binder_objstat_strings[i],
				   obj_created - obj_deleted - obj_zombie,
				   obj_zombie, obj_created);
	}
}
...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, &binder_devices, hlist) {
		...
		if (do_lock)
			binder_lock(context, __func__);
		mutex_lock(&context->binder_dead_nodes_lock);
		...
		hlist_for_each_entry(node, &context->binder_dead_nodes,
				     dead_node)
			_print_binder_node(m, node);
		mutex_unlock(&context->binder_dead_nodes_lock);
...
static int __init init_binder_device(const char *name)
{
	...
	mutex_init(&context->binder_procs_lock);
	mutex_init(&context->binder_dead_nodes_lock);
	mutex_init(&context->binder_context_mgr_node_lock);
...
static int __init binder_init(void)
{
	...
	atomic64_set(&binder_seq_count, 0);
	for (i = 0; i < SEQ_BUCKETS; i++) {
