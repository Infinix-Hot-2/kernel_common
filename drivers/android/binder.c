...
static char *binder_devices = CONFIG_ANDROID_BINDER_DEVICES;
module_param_named(devices, binder_devices, charp, 0);
					      ...
					      binder_size_t *failed_at)
{
	...
	binder_size_t num_validated_objects = 0;
	...
	for (; offp < off_end; offp++, num_validated_objects++) {
		struct binder_object_header *hdr;
			       ...
			       binder_size_t last_fixup_min_off)
{
	...
	*(uintptr_t *)(parent_buffer + bp->parent_offset) = bp->buffer;
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	binder_size_t num_objects_validated = 0;
	struct binder_context *context = proc->context;
	...
	off_min = 0;
	for (; offp < off_end; offp++, num_objects_validated++) {
		struct binder_object_header *hdr;
		...
		case BINDER_TYPE_PTR: {
			...
			if (ALIGN(bp->length, sizeof(u64)) > buf_left) {
