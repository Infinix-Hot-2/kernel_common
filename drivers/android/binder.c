...
static int binder_inc_node(struct binder_proc *proc, struct binder_node *node,
			  ...
			  struct list_head *target_list)
{
	...
	if (strong) {
		if (ref->strong == 0) {
			ret = binder_inc_node(ref->proc, ref->node, 1,
					      1, target_list);
			if (ret)
				return ret;
		}
		...
		if (ref->weak == 0) {
			ret = binder_inc_node(ref->proc, ref->node, 0,
					      1, target_list);
			if (ret)
				return ret;
		}
				     ...
				     struct binder_transaction *in_reply_to)
{
	...
	for (fdi = 0; fdi < fda->num_fds; fdi++) {
		int target_fd = binder_translate_fd(fd_array[fdi], t,
						   thread, in_reply_to);
		if (target_fd < 0)
			break;
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (target_node)
		binder_inc_node(proc, target_node, 1, 0, NULL);
