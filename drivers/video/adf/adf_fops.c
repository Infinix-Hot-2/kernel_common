		...
		struct adf_buffer_config __user *cfg, struct adf_buffer *buf)
{
	...
	if (user_buf.acquire_fence >= 0) {
		buf->acquire_fence = sync_file_get_fence(user_buf.acquire_fence);
...
{
	struct sync_file *complete_fence_file;
	...
	complete_fence_file = sync_file_create(complete_fence);
	if (!complete_fence_file) {
		ret = -EFAULT;
		goto err_put_complete_fence;
	}

	fd_install(complete_fence_fd, complete_fence_file->file);
	return 0;

err_put_complete_fence:
	fence_put(complete_fence);
