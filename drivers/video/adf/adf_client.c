...
static struct fence *adf_sw_complete_fence(struct adf_device *dev)
{
	...
	dev->timeline_max++;
	complete_fence = sw_sync_fence_create(dev->timeline, dev->timeline_max);
	if (!complete_fence)
 ...
 */
struct fence *adf_device_post(struct adf_device *dev,
 ...
 */
struct fence *adf_device_post_nocopy(struct adf_device *dev,
		struct adf_interface **intfs, size_t n_intfs,
 ...
 * from the screen.  On failure, returns ERR_PTR(-errno).
 */
struct fence *adf_interface_simple_post(struct adf_interface *intf,
