...
static void adf_fence_wait(struct adf_device *dev, struct fence *fence)
{
	/* fence_wait_timeout() dumps debug information on timeout.  Experience
	...
	int err = fence_wait_timeout(fence, false, ADF_SHORT_FENCE_TIMEOUT);
	if (err >= 0)
		return;

	if (err == -ETIME)
		err = fence_wait_timeout(fence, false, ADF_LONG_FENCE_TIMEOUT);
...
static void adf_post_work_func(struct kthread_work *work)
{
	...
	list_for_each_entry_safe(post, next, &saved_list, head) {
		...
		for (i = 0; i < post->config.n_bufs; i++) {
			struct fence *fence = post->config.bufs[i].acquire_fence;
			if (fence)
...
void adf_device_destroy(struct adf_device *dev)
{
	...
	if (dev->timeline)
		sw_sync_timeline_put(dev->timeline);
