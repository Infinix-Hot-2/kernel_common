...
static void prdebug_full_state(int flag, int indent_level, const char *fmt, ...)
{
	va_list args;
	char *fmt_buff;
	char *buff;
	...
	/*
	 * if the call is from proc_ctrl_show, the &sock_tag_list_lock
	 * is already locked when seq_file start. Recursively lock it will
	 * cause deadlock here
	 */
	if(flag == QTUDEV)
		spin_lock_bh(&sock_tag_list_lock);
...
static int qtaguid_ctrl_proc_show(struct seq_file *m, void *v)
{
	...
	if (sock_tag_entry != SEQ_START_TOKEN) {
		...
		prdebug_full_state(CTRL, 0, "proc ctrl");
	}
...
static int qtudev_release(struct inode *inode, struct file *file)
{
	...
	prdebug_full_state(QTUDEV, 0, "%s(): pid=%u tgid=%u", __func__,
			   current->pid, current->tgid);
	return 0;
}
