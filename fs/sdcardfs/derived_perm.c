...
void get_derived_permission_new(struct dentry *parent, struct dentry *dentry, const struct qstr *name)
{
	...
	switch (parent_info->perm) {
		...
		case PERM_ROOT:
			...
			break;
		case PERM_ANDROID:
			if (qstr_eq(name, &q_data)) {
				/* App-specific directories inside; let anyone traverse */
				info->perm = PERM_ANDROID_DATA;
				set_top(info, &info->vfs_inode);
			} else if (qstr_eq(name, &q_obb)) {
				/* App-specific directories inside; let anyone traverse */
				info->perm = PERM_ANDROID_OBB;
				info->under_obb = true;
				set_top(info, &info->vfs_inode);
				/* Single OBB directory is always shared */
			} else if (qstr_eq(name, &q_media)) {
				/* App-specific directories inside; let anyone traverse */
 		...
 		if(!di->lower_path.dentry) {
			...
			if(!path_buf) {
				...
				if (d_unhashed(di->lower_path.dentry) ||
					!str_eq(sbi->obbpath_s, obbpath_s)) {
					ret = 1;
				}
				kfree(path_buf);
			}
